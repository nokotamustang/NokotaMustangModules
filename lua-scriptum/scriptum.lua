...
local sub_pattern_code = '`' .. anyText
...
local function systemCheck()
  ...
  if check == '\\' or check == '\\\\' then return 'windows' end
  return 'linux'
end

local function stringReplace(stringIn, tag, replace)
  return stringIn:gsub(tag, replace)
end

local function firstToUpper(text) return (text:gsub('^%l', string.upper)) end
  ...
  for _, v in ipairs(filesTable) do
    ...
    if info then
      if info.type == 'file' then
        ...
        fileTree[#fileTree + 1] = full_path
      else
        if info.type == 'directory' then
          fileTree = recursiveFileSearch(full_path, fileTree)
        end
      end
    end
  end
...
local function scanDir(folder, fileTree)
  ...
  if pfile == nil then
    print("error: failed to scan folder '" .. folder .. "' (scanDir)")
    return fileTree
  end
  for item in pfile:lines() do fileTree[#fileTree + 1] = folder .. '/' .. item end
  pfile:close()
  -- Folders --
  if systemCheck() == 'windows' then
...
local function searchForPattern(lines, startLine, forLines, pattern)
  ...
  for j = 1, forLines do
    ...
    if k <= count then
      ...
      if line3 then return j, line3 end
    end
  end
  return nil, nil
end
...
local function extractParam(fnSet, lines, startLine, j)
  ...
  if match and line then
    if not fnSet.pars then fnSet.pars = {} end
    local par = {}
...
local function extractReturn(fnSet, lines, startLine, j)
  ...
  if match and line then
    if not fnSet.returns then fnSet.returns = {} end
    local ret = {}
...
local function extractUnpack(fnSet, lines, startLine, j)
  ...
  if match and line then
    local ret = {}
    if not fnSet.unpack then fnSet.unpack = {} end
    ret.name = removeLeadingSpaces(line)
    local findUnpack = searchForPattern(lines, 1, #lines,
      'local ' .. line .. ' = {')
    if findUnpack then
...
local function extractFunctionBlock(lines, startLine, data)
  local search2 = searchForPattern(lines, startLine, 1,
    patternInsideBlockComment)
  if search2 then
    data.api[#data.api + 1] = { line = startLine }
  else
    local search2b, result2b = searchForPattern(lines, startLine, 1,
      startBlockComment)
    if search2b then
      ...
      -- Function details --
      if search3 then
        local fnL, fnLine = searchForPattern(lines, startLine + search3, 1,
          patternFunction)
        if fnL then fnSet.name = fnLine end
        for j = 1, search3 do
...
local function multiLineField(set, field, data)
  ...
  if text ~= '' then set[field][#set[field] + 1] = text end
end

local function catchMultilineEnd(set, multi_lines, multilineStarted)
  for i = 1, #multi_lines do
    set[multilineStarted][#set[multilineStarted] + 1] = multi_lines[i]
  end
end

local function searchForMultilineTaggedData(set, line, multi_lines,
                                            multilineStarted)
  local description = string.match(line, patternDesc)
  if description then
    if multilineStarted then
      catchMultilineEnd(set, multi_lines, multilineStarted)
    end
    ...
    if search3 then
      ...
      for j = 1, search3 - 2 do
        ...
        if paramLineN then -- Line is prefixed with '@' --
          ...
          local matched = searchForMultilineTaggedData(set, line, multi_lines,
            multiline_started)
          if matched then
            multiline_started = matched
            multi_lines = {}
          else
            local otherTagMatch = searchForTaggedData(line, set)
            if otherTagMatch and multiline_started then
              catchMultilineEnd(set, multi_lines, multiline_started)
              multiline_started = nil
              multi_lines = {}
            end
          end
        else -- Line is not prefixed with '@' --
          local line = lines[startLine + j + 1]
          if multiline_started then
            local text = removeLeadingSpaces(line)
            multi_lines[#multi_lines + 1] = text
          end
        end
      end
      if multiline_started then -- On end block, but check if a multiline catch wasn't done --
        catchMultilineEnd(set, multi_lines, multiline_started)
      end
    end
    data.header = set
    ...
    if set[field] then
      ...
      if type(set[field]) == 'table' then
        ...
        for j = 1, maximum do
          ...
          text = stringReplace(text, '%(start%)', '--[[')
          text = stringReplace(text, '%(end%)', ']]')
          count = count + 1
          if text == '||' then
            output:write('\n')
            output:write('\n**' .. firstToUpper(field) .. '**:')
            ...
            local code = string.match(text, sub_pattern_code)
            if code then
              if count == 2 then output:write('\n') end
              output:write('\n    ' .. code)
              codeBlockOpened = true
            else
              if codeBlockOpened then codeBlockOpened = false end
              output:write('\n' .. text)
            end
          end
        end
...
local function stripOutRoot(text)
  if rootInput == '' then return text end
  local cleanRootInput = rootInput
  cleanRootInput = stringReplace(cleanRootInput, '\\\\', '/')
  cleanRootInput = stringReplace(cleanRootInput, '\\', '/')
  text = stringReplace(text, cleanRootInput .. '/', '')
  text = stringReplace(text, cleanRootInput, '')
  return text
end
...
local function outputMDFile(file)
  ...
  outFilename = stringReplace(outFilename, config.codeSourceType, '')
  return outFilename
end
...
local function printParams(fileWriter, v3)
  for _, v4 in pairs(v3.pars) do
    local text2 = '> &rarr; '
    if v4.name then text2 = text2 .. '**' .. v4.name .. '**' end
    if v4.typing then text2 = text2 .. ' (' .. v4.typing .. ')' end
    if v4.default then text2 = text2 .. ' <*' .. v4.default .. '*>' end
    if v4.note then text2 = text2 .. ' `' .. v4.note .. '`' end
    ...
    if v4.lines then
      for i = 1, #v4.lines do
        ...
        if comment1 then
          ...
          local stripped = stringReplace(line, comment1, '')
          stripped = stringReplace(stripped, commaComment, '')
          stripped = removeLeadingSpaces(stringReplace(stripped, '-', ''))
          fileWriter:write(' `' .. stripped .. '`')
          fileWriter:write('  \n')
        elseif comment2 then
          fileWriter:write('> - ' .. removeLeadingSpaces(comment2))
          local stripped = stringReplace(line, comment2, '')
          stripped = stringReplace(stripped, comment, '')
          stripped = removeLeadingSpaces(stringReplace(stripped, '-', ''))
          fileWriter:write(' `' .. stripped .. '`')
          fileWriter:write('  \n')
        else
          fileWriter:write('> - ' ..
            removeLeadingSpaces(stringReplace(line, ',', '')))
          fileWriter:write('  \n')
        end
      end
    end
...
local function generateDoc(data)
  ...
  for _, v2 in pairs(data.requires) do
    ...
    if module.fileData[file] then isInternal = true end
    if isInternal then
      fileWriter:write('\n+ [' .. name .. '](' .. link .. ')')
    else
      fileWriter:write('\n+ ' .. name .. '')
  ...
  if hasREQ then fileWriter:write('\n') end
  ...
  for _, v3 in pairs(data.api) do
    if v3.name then
      ...
      count = count + 1
      local nameText = stringReplace(v3.name, 'module.', '')
      fileWriter:write('\n**' .. removeLeadingSpaces(nameText) .. '**')
      if v3.pars then printFn(fileWriter, v3) end
      if v3.desc then
        fileWriter:write('\n')
        fileWriter:write('> ' .. v3.desc)
        fileWriter:write('  \n')
      end
      if v3.pars then printParams(fileWriter, v3) end
      if v3.unpack then printUnpack(fileWriter, v3) end
      if v3.returns then printReturns(fileWriter, v3) end
    end
  end
  fileWriter:write('\n# Project\n')
  fileWriter:write('\n+ [' .. toRoot .. '](README.md)')
...
local function parseFile(file)
  ...
  for i = 1, count do
    if i == 1 then extractHeaderBlock(lines, 0, data) end
    if not data.header or
        (not data.header.endHeader or i > data.header.endHeader) then
...
function module.start(rootPath, outputPath)
  -- Prep --
  if rootPath then rootInput = rootPath end
  if outputPath then outPath = outputPath end
  prepareOutput()
