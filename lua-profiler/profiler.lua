...
local formatFunLine, formatFunTime, formatFunRelative, formatFunCount, divider,
nilTime
...
local function rebuildColumnPatterns()
  local c = config
  local str = 's: %-'
  outputHeader = '| %-' .. c.fW .. str .. c.fnW .. str .. c.lW .. str .. c.tW ..
      str .. c.rW .. str .. c.cW .. 's|\n'
  formatHeader = string.format(outputHeader, 'FILE', 'FUNCTION', 'LINE', 'TIME',
    '%', '#')
  outputTitle = '%-' .. c.fW .. '.' .. c.fW .. str .. c.fnW .. '.' .. c.fnW ..
      str .. c.lW .. 's'
...
local function functionReport(information)
  ...
  local title = string.format(outputTitle, src, name, string.format(
    formatFunLine, information.line_defined or 0))
  local report = reportCache[title]
  if not report then
    report = {
      title = string.format(outputTitle, src, name, string.format(formatFunLine,
        information.line_defined or
        0)),
      count = 0,
      timer = 0
    }
...
function module.report(filename)
  if stopTime == -1 then module.stop() end
  ...
  if fileWriter == nil then
    print("error: failed to create '" .. filename .. "' (report)")
    return
  end
  local divide = false
  ...
  for i = 1, reportCount do
    ...
    if funcReport.count > 0 and funcReport.timer <= totalTime then
      ...
      if printThis then
        ...
        local relTime = string.format(formatFunRelative,
          (funcReport.timer / totalTime) * 100)
        ...
        -- Build final line
        local output = string.format(formatOutput, funcReport.title, timer,
          relTime, count)
